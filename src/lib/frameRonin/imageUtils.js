// FrameRonin-main/FrameRonin-main/frontend/src/components/ParamsStep/utilsResizeCanvasLegacy.ts
async function resizeImageToBlobCanvasLegacy(blob, targetW, targetH, keepAspect = true, pixelated = false) {
...
async function resizeImageToBlob(blob, targetW, targetH, keepAspect = true, pixelated = false) {
  if (pixelated) {
...
async function extendImageBottom(blob, _bottomPx) {
  const img = await (typeof createImageBitmap === "function" ? createImageBitmap(blob) : new Promise((resolve, reject) => {
    const im = new Image();
    const url = URL.createObjectURL(blob);
    im.onload = () => {
...
async function processAncientCostumeAllActions(blob) {
  const img = await (typeof createImageBitmap === "function" ? createImageBitmap(blob) : new Promise((resolve, reject) => {
    const im = new Image();
    const url = URL.createObjectURL(blob);
    im.onload = () => {
  ...
  const getCell = (row, col) => {
    ...
    ctx2.drawImage(img, sx, sy, cellSize, cellSize, pad.left, pad.top, cellSize, cellSize);
    return canvas;
  };
  const flipH = (src) => {
    ...
    ctx2.scale(-1, 1);
    ctx2.drawImage(src, 0, 0, src.width, src.height, 0, 0, src.width, src.height);
    ctx2.restore();
    return c;
  };
  const cells = {};
  ...
  let y = 0;
  const draw = (key, flipped, count, xStart = 0) => {
...
async function resizeFrameToBlob(blob, targetW, targetH, padding) {
  const img = await (typeof createImageBitmap === "function" ? createImageBitmap(blob) : new Promise((resolve, reject) => {
    const im = new Image();
    const url = URL.createObjectURL(blob);
    im.onload = () => {
...
async function applyInnerStroke(blob, strokeWidth, strokeColor) {
...
function composeSpriteSheetClient(frames, timestamps, targetW, targetH, padding, spacing, columns, resizeFn) {
  return new Promise((resolve, reject) => {
    ...
    const processFrame = (i) => {
      ...
      img.onload = () => {
        ...
        framesIndex.push({
          ...
          w: targetW,
          h: targetH,
          t: Math.round((timestamps[i] ?? 0) * 1e3) / 1e3
        });
        loaded++;
...
function applyChromaKeyHybridTolerance(dataUrl, bgR, bgG, bgB, contiguousTolerance, nonContiguousTolerance, feather) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.onload = () => {
      ...
      const matchContiguous = (i) => dist(i) <= contiguousTolerance;
      const contiguous = /* @__PURE__ */ new Set();
      const start = idx(0, 0);
...
function applyChromaKeyAdaptiveRegion(dataUrl, bgR, bgG, bgB, contiguousTolerance, nonContiguousTolerance, largeRegionThreshold, largeRegionToleranceBonus, feather) {
  void feather;
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.onload = () => {
      ...
      const nonContiguousCandidates = /* @__PURE__ */ new Set();
      ...
      const pixelToRegion = /* @__PURE__ */ new Map();
      const regionSizes = [];
      const vis = /* @__PURE__ */ new Set();
      const dx = [0, 1, 0, -1];
      const dy = [-1, 0, 1, 0];
      ...
      const effectiveTol = (rId) => regionSizes[rId] > largeRegionThreshold ? nonContiguousTolerance + largeRegionToleranceBonus : nonContiguousTolerance;
      for (let i = 0; i < d.length; i += 4) {
        const dst = Math.sqrt(
          (d[i] - bgR) ** 2 + (d[i + 1] - bgG) ** 2 + (d[i + 2] - bgB) ** 2
        );
        if (contiguous.has(i)) {
          ...
          const tol = rId !== void 0 ? effectiveTol(rId) : nonContiguousTolerance;
          if (dst <= tol) d[i + 3] = 0;
          else if (feather > 0 && dst < tol + feather) {
            const t = (dst - tol) / feather;
            d[i + 3] = Math.round(255 * Math.min(1, t));
...
async function analyzeDuplicateFrames(frames) {
  ...
  const hashToIndices = /* @__PURE__ */ new Map();

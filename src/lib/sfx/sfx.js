...
var PHASER_BUFFER_SIZE = 2048;
var PD_COSTABLE_SIZE = 2048;
var PD_COSTABLE = createPdCosTable();
var MAX_FREQUENCY = 12e3;
var MIN_DURATION_SECONDS = 0.18;
function clampSfxParams(params) {
  const merged = {
    ...DEFAULT_SFX_PARAMS,
    ...params
  };
...
function serializeSfxParams(params) {
  return JSON.stringify(params, null, 2);
}
function deserializeSfxParams(serialized) {
  const parsed = JSON.parse(serialized);
  return clampSfxParams(parsed);
}
function mutateSfxParams(current) {
...
function createSfxPreset(preset, current = DEFAULT_SFX_PARAMS) {
  switch (preset) {
    case "random":
      return clampSfxParams({
...
function renderSfx(params, options) {
  ...
  const initialSlide = signedCurve(safeParams.slide) * 8e-5;
  const slideDelta = signedCurve(safeParams.deltaSlide) * 8e-8;
  const initialDuty = clamp(0.05 + safeParams.squareDuty * 0.9, 0.05, 0.95);
  const dutyDelta = safeParams.dutySweep * 4e-5;
  const vibratoRate = 0.5 + safeParams.vibratoSpeed * 18;
  const vibratoDepth = safeParams.vibratoDepth * 0.35;
  const repeatLimit = safeParams.repeatSpeed > 0.01 ? Math.max(128, Math.floor(options.sampleRate * (0.12 + (1 - safeParams.repeatSpeed) * 1.48))) : 0;
  const changeLimit = safeParams.changeSpeed > 0.01 ? Math.max(128, Math.floor(options.sampleRate * (0.05 + (1 - safeParams.changeSpeed) * 1.3))) : 0;
  const changeMultiplier = safeParams.changeAmount >= 0 ? 1 + safeParams.changeAmount * 1.5 : 1 / (1 + Math.abs(safeParams.changeAmount) * 1.5);
  ...
  const lowPassSweep = safeParams.lowPassSweep * 2e-5;
  ...
  const highPassSweep = safeParams.highPassSweep * 2e-5;
  ...
  for (let index = 0; index < totalSamples; index += 1) {
    ...
    lowPassCutoff = clamp(lowPassCutoff + lowPassSweep, 1e-3, 1);
...
function renderFootsteppr(params, options) {
  ...
  const outputSignal = options.sampleRate === internalSampleRate ? boostedSignal : resampleSignal(boostedSignal, internalSampleRate, options.sampleRate);
...
function encodeSfxWav(samples, sampleRate, bitDepth) {
...
function getWaveformSample(waveform, phase, duty, noiseValue) {
  switch (waveform) {
    case "square":
...
function getEnvelopeLevel(index, attackSamples, sustainSamples, decaySamples, sustainPunch) {
  ...
  if (index < sustainEnd) {
    const sustainProgress = sustainSamples <= 1 ? 1 : (index - sustainStart) / sustainSamples;
    return 1 + (1 - sustainProgress) * sustainPunch * 0.8;
  }
  const decayStart = sustainEnd;
  const decayEnd = decayStart + decaySamples;
  if (index < decayEnd) {
    const decayProgress = decaySamples <= 1 ? 1 : (index - decayStart) / decaySamples;
    return Math.max(0, 1 - decayProgress);
  }
  return 0;
}
function createFootstepEnvelopeSignal(totalSamples, sampleRate, params) {
  ...
  const baseEnvelope = (time) => heelEnvelope(time) + rollEnvelope(time) + ballEnvelope(time);
...
function renderFootstepTerrain(terrain, envelope, sampleRate) {
  switch (terrain) {
    case "snow":
...
function renderSnowTerrain(envelope, sampleRate) {
  ...
  const sweep = pdAdd(pdMulScalar(envelope, 9e3), pdConstant(envelope.length, 700));
...
function renderGrassTerrain(envelope, sampleRate) {
  ...
  const noiseLow2000 = pdLop(rustleNoise, pdConstant(envelope.length, 2e3), sampleRate);
  ...
  const rustleInput = pdClip(pdMulScalar(rustleFourth, 1e-5), -0.9, 0.9);
...
function renderDirtTerrain(envelope, sampleRate) {
  const envelopeSquared = pdMul(envelope, envelope);
  const envelopeFourth = pdMul(envelopeSquared, envelopeSquared);
...
function renderGravelTerrain(envelope, sampleRate) {
  const gravelNoise = pdNoise(envelope.length);
  const low300 = pdLop(gravelNoise, pdConstant(envelope.length, 300), sampleRate);
  const low2000 = pdLop(gravelNoise, pdConstant(envelope.length, 2e3), sampleRate);
  ...
  const gritSweep = pdClip(
    pdAdd(
      pdMulScalar(pdLop(gravelNoise, pdConstant(envelope.length, 50), sampleRate), 5e4),
      pdMulScalar(envelope, 1e3)
    ),
    500,
    1e4
  );
...
function resizeFn(fn, inputStart, inputEnd, outputStart, outputEnd) {
  return (time) => fn(
    (time - outputStart) / (outputEnd - outputStart) * (inputEnd - inputStart) + inputStart
  );
}
function pdConstant(length, value) {
  const signal = new Float32Array(length);
  signal.fill(value);
  return signal;
}
function pdNoise(length) {
  const signal = new Float32Array(length);
...
function pdMulScalar(signal, value) {
...
function pdClip(signal, min, max) {
  const result = new Float32Array(signal.length);
  for (let index = 0; index < signal.length; index += 1) {
...
function pdOsc(frequencySignal, sampleRate) {
...
function pdLop(signal, cutoffSignal, sampleRate) {
  const result = new Float32Array(signal.length);
  const conversionFactor = 2 * Math.PI / sampleRate;
  let last = signal[0] ?? 0;
  for (let index = 0; index < signal.length; index += 1) {
...
function pdHip(signal, cutoffSignal, sampleRate) {
  const result = new Float32Array(signal.length);
  const conversionFactor = 2 * Math.PI / sampleRate;
  let last = signal[0] ?? 0;
  for (let index = 0; index < signal.length; index += 1) {
...
function pdVcf(signal, resonanceFrequencySignal, qSignal, sampleRate) {
  const result = new Float32Array(signal.length);
  const conversionFactor = 2 * Math.PI / sampleRate;
  let real = 0;
  let imaginary = 0;
  for (let index = 0; index < signal.length; index += 1) {
    ...
    real = amplitudeCorrection * oneMinusResonance * (signal[index] ?? 0) + coefficientReal * previousReal - coefficientImaginary * imaginary;
...
function pdSinLookup(angle) {
  const shiftedAngle = angle - Math.PI / 2;
  return pdCosLookup(shiftedAngle);
}
function resampleSignal(signal, fromSampleRate, toSampleRate) {
...
function isFootstepTerrain(value) {
